...
export const dimRestaurantSettings = mysqlTable('dim_restaurant_settings', {
  settingId: int('setting_id').primaryKey().autoincrement(),
  restaurantId: int('restaurant_id').notNull(),
  lowStockThreshold: int('low_stock_threshold').default(40), // Default 40%
  createdAt: timestamp('created_at').defaultNow(),
  updatedAt: timestamp('updated_at').defaultNow().onUpdateNow(),
}, (table) => ({
  restaurantIdIdx: index('settings_restaurant_id_idx').on(table.restaurantId),
}));
...
// Metrics tables
export const factRestaurantMetrics = mysqlTable('fact_restaurant_metrics', {
  metricId: int('metric_id').primaryKey().autoincrement(),
  restaurantId: int('restaurant_id').notNull(),
  date: date('date').notNull(),
  inventoryTurnover: decimal('inventory_turnover', { precision: 10, scale: 2 }),
  avgPrepTimeHours: decimal('avg_prep_time_hours', { precision: 10, scale: 2 }),
  overallIngredientPriceAvg: decimal('overall_ingredient_price_avg', { precision: 10, scale: 2 }),
  monthlyCleaningCompletionPct: decimal('monthly_cleaning_completion_pct', { precision: 5, scale: 2 }),
  weeklyCleaningCompletionPct: decimal('weekly_cleaning_completion_pct', { precision: 5, scale: 2 }),
  createdAt: timestamp('created_at').default(sql`CURRENT_TIMESTAMP`),
  updatedAt: timestamp('updated_at').default(sql`CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP`),
}, (table) => ({
  restaurantDateIdx: index('restaurant_date_idx').on(table.restaurantId, table.date),
  restaurantIdIdx: index('restaurant_id_idx').on(table.restaurantId),
}));

// Prep list completion fact table
export const factPrepCompletion = mysqlTable('fact_prep_completion', {
  completionId: int('completion_id').primaryKey().autoincrement(),
  restaurantId: int('restaurant_id').notNull(),
  dateId: int('date_id').notNull(),
  completionTime: timestamp('completion_time').notNull(),
  createdAt: timestamp('created_at').default(sql`CURRENT_TIMESTAMP`),
}, (table) => ({
  restaurantDateIdx: index('prep_restaurant_date_idx').on(table.restaurantId, table.dateId),
}));

// Cleaning task dimension table
export const dimCleaningTask = mysqlTable('dim_cleaning_task', {
  taskId: int('task_id').primaryKey().autoincrement(),
  restaurantId: int('restaurant_id').notNull(),
...
// Cleaning task completion fact table
export const factCleaningCompletion = mysqlTable('fact_cleaning_completion', {
  completionId: int('completion_id').primaryKey().autoincrement(),
  restaurantId: int('restaurant_id').notNull(),
  ...
  restaurantTaskDateIdx: index('restaurant_task_date_idx').on(table.restaurantId, table.taskId, table.date),
  taskIdIdx: index('task_id_idx').on(table.taskId),
}));

// Inventory transaction fact table to track all inventory changes
export const factInventoryTransaction = mysqlTable('fact_inventory_transaction', {
  ...
  transactionType: varchar('transaction_type', { length: 50 }).notNull(), // 'prep', 'order', 'adjustment', 'waste'
  referenceId: int('reference_id'), // ID of the related entity (prep item, order, etc.)
  previousQuantity: decimal('previous_quantity', { precision: 10, scale: 2 }),
  newQuantity: decimal('new_quantity', { precision: 10, scale: 2 }),
  notes: text('notes'),
  createdBy: int('created_by'),
  createdAt: timestamp('created_at').default(sql`CURRENT_TIMESTAMP`),
}, (table) => ({
  ingredientIdIdx: index('transaction_ingredient_id_idx').on(table.ingredientId),
  restaurantIdIdx: index('transaction_restaurant_id_idx').on(table.restaurantId),
  dateIdIdx: index('transaction_date_id_idx').on(table.dateId),
